...
const getDownloadFileNameLeagueDetail = (leagueName: string) => `${leagueName ?? ""}_Detail_Info_${dayjs().format(FORMAT_DATE.FILE_DOWNLOAD)}`;
// get download file name off roulette detail
const getDownloadFileNameRouletteDetail = (rouletteName: string) => `${rouletteName ?? ""}_Detail_Info_${dayjs().format(FORMAT_DATE.FILE_DOWNLOAD)}`;
//  get download file name off balloon detail
const getDownloadFileNameBalloonDetail = (eventName: string) => `${eventName ?? ""}_Balloon_Detail_Info_${dayjs().format(FORMAT_DATE.FILE_DOWNLOAD)}`;
...
const getDownloadFileNameHoleInOneList = () => `Event_Hole_In_One_List_${dayjs().format(FORMAT_DATE.FILE_DOWNLOAD)}`;
// get download file name off push list
const getDownloadFileNamePushList = () => `Push_List_${dayjs().format(FORMAT_DATE.FILE_DOWNLOAD)}`;
// get download file name off location list
const getDownloadFileNameLocationList = () => `Location_List_${dayjs().format(FORMAT_DATE.FILE_DOWNLOAD)}`;
